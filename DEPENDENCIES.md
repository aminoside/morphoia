...
| setuptools 83.0.0, wheel 0.47.0, and packaging 26.3 | Python distribution build backend | Exact versions and wheel hashes are isolated in `wheel-build-e1.lock`; the installed-wheel smoke exercises them without changing the closed Engine test lock | Development/build-only; byte-for-byte wheel reproducibility, component license review, and vulnerability analysis remain NOT_RUN |
